...
workflow mocha {
  input {
    ...
    String iaap_cli_docker = "us.gcr.io/mccarroll-mocha/iaap_cli:1.11-dev"
    String autoconvert_docker = "us.gcr.io/mccarroll-mocha/autoconvert:1.11-dev"
    String apt_docker = "us.gcr.io/mccarroll-mocha/apt:1.11-dev"
    String gtc2vcf_docker = "us.gcr.io/mccarroll-mocha/mocha:1.11-dev"
    String bcftools_docker = "us.gcr.io/mccarroll-mocha/mocha:1.11-dev"
    String shapeit4_docker = "us.gcr.io/mccarroll-mocha/shapeit4:1.11-dev"
    String eagle_docker = "us.gcr.io/mccarroll-mocha/eagle:1.11-dev"
    String mocha_docker = "us.gcr.io/mccarroll-mocha/mocha:1.11-dev"
    String mocha_plot_docker = "us.gcr.io/mccarroll-mocha/mocha_plot:1.11-dev"
  ...
  scatter (key in ["batch_id", "vcf", "vcf_index", "xcl_vcf", "xcl_vcf_index", "pgt_vcf", "pgt_vcf_index"]) { Array[String] output_tsv_cols = output_map[key] }
  call write_tsv {
    input:
      tsv = flatten([[["batch_id", "vcf", "vcf_index", "xcl_vcf", "xcl_vcf_index", "pgt_vcf", "pgt_vcf_index"]], transpose(output_tsv_cols)]),
...
task vcf_scatter {
  ...
  output {
    ...
    File? extra_vcf_file = if defined(extra) then "vcfs/" + filebase + "." + extra + ".bcf" else None
  }
...
task vcf_phase {
  ...
  Float mult = select_first([mult_override, if eagle then 1.5 else 9.0])
  Float memory = select_first([memory_override, 3.5 + mult * n_vars * (n_smpls + select_first([n_panel_smpls, 0])) / 1024 / 1024 / 1024])
...
task vcf_concat {
  ...
  command <<<
    ...
    ~{if defined(ref_fasta_fai) then
      ...
      "  \"" + filebase + ".tmp.bcf\"\n" +
      "rm \"" + filebase + ".tmp.bcf\"\n" +
      "rm \"" + basename(select_first([ref_fasta_fai])) + "\""
    else ""}
    bcftools index --force "~{filebase}.bcf"
  >>>
