# Association between SPDL1 and mCA events (Using "aoxing-mocha" VM)
...
# SPDL1
# system("awk 'NF==2' finngen_R6_b01_b51.rs116483731.vcf > finngen_R6_b01_b51.rs116483731_format.vcf")
snp <- read.table("finngen_R6_b01_b51.rs116483731_format.vcf", header=F)
...
for (i in c("Autosomes","Large_Autosomes","AnyChr","Large_AnyChr")){
	for (k in c("ALL")){
		...
		for (j in c(1,3)){
			...
			if (j==1){			
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SEX+SMOKE2")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2","SEX","SMOKE2")	
			} else if (j==2){
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SEX+SMOKE2+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2","SEX","SMOKE2","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10")	
			} else if (j==3){
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SEX")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2","SEX")	
			} else {
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SEX+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10")
...
for (i in c("ChrY","Large_ChrY","Autosomes","Large_Autosomes","AnyChr","Large_AnyChr")){
	for (k in c("male")){
		...
		for (j in c(1,3)){
			...
			if (j==1){			
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SMOKE2")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2","SMOKE2")	
			} else if (j==2){
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+SMOKE2+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2","SMOKE2","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10")	
			} else if (j==3){
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2")
				mod[,"Variable"] <- c("intercept","snp","BL_AGE","BL_AGE_2")	
			} else {
				mod[,"Model"] <- paste0(i, "=rs116483731+AGE+AGE_2+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10")
...
write.csv(summaryDF, "summaryDF_noPC.csv", row.names=F)
